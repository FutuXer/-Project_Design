...
void Hero::pickUpItems(std::string Name, int Num) {
    int positionInbag = -1;                                // 把掉落的物品移动到背包中
    if (findItemCount(Name, positionInbag) > 0 && Name[0]!='2' && Name[0] != '3') {          // 已有,数量加一    // 后两个条件是希望兼容合成物品时的添加
        items.at(positionInbag)->setNum(items.at(positionInbag)->getItemNum() + Num);
        items.at(positionInbag)->setCameraMask((unsigned short)CameraFlag::USER1);
        if (positionInbag < 8) {
            pocketItems.at(positionInbag)->setNum(pocketItems.at(positionInbag)->getItemNum() + Num);
        ...
        for (int i = 0; i < items.size(); ++i) {
            if (items.at(i)->getItemName() == "0.png") {
                // 尚未获得，把新物品放在第一个空位置上
                auto newItem = MyItem::create(Name, Num);
    ...
    if ((Name[0] == '1' || Name[0] == '5') && Name != "10.png" && Name != "11.png")
