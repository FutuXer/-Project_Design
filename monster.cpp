...
void NPC::addPhy() {
    ...
    npcbody->setCategoryBitmask(0x04);
...
void NPC::addTouchListener()
{
    ...
    listener->onTouchBegan = [this](Touch* touch, Event* event)
        {
            auto location = touch->getLocation(); // 屏幕坐标

            // 打印点击位置 (屏幕坐标)
            CCLOG("Touched at: (%.2f, %.2f)", location.x, location.y);
            ...
            if (hero)
            {
                // 获取NPC的场景坐标
                auto spritePos = this->getPosition();
                //CCLOG("NPC Position (scene): (%.2f, %.2f)", spritePos.x, spritePos.y);

                // 获取主角的场景坐标
                auto heroPos = hero->getPosition();
                //CCLOG("Hero Position (scene): (%.2f, %.2f)", heroPos.x, heroPos.y);

                // 计算 NPC 和主角之间的场景坐标距离
                float deltaX = std::abs(spritePos.x - heroPos.x);
                float deltaY = std::abs(spritePos.y - heroPos.y);
                //CCLOG("DeltaX: %.2f, DeltaY: %.2f", deltaX, deltaY);

                // 如果水平和竖直方向距离都小于 50 像素，则显示交互菜单
...
void NPC::showInteractionMenu()
{
    ...
    // 创建互动按钮
    auto startLabel = Label::createWithTTF(u8"互动", "fonts/jiantisongti.ttf", 24);
    auto startButton = MenuItemLabel::create(
        startLabel,
        CC_CALLBACK_1(NPC::button1, this)
    );
    ...
    // 创建菜单并将按钮添加到菜单中
    interactionMenu = Menu::create(startButton, exitButton, nullptr);

    // 获取NPC的世界坐标（相对于窗口左下角的坐标）
    auto spritePos = this->getPosition();
    auto screenSpritePos = this->getParent()->convertToWorldSpace(spritePos);
    ...
    // 设置互动按钮的位置
    startButton->setPosition(finalSpriteScreenPos.x , finalSpriteScreenPos.y + 60);

    // 设置退出按钮的位置（让它在互动按钮旁边）
    exitButton->setPosition(finalSpriteScreenPos.x+60 , finalSpriteScreenPos.y + 60);

    // 设置菜单的摄像头掩码（确保菜单由USER1相机渲染）
    interactionMenu->setCameraMask((unsigned short)CameraFlag::USER1);
    startButton->setCameraMask((unsigned short)CameraFlag::USER1);
    exitButton->setCameraMask((unsigned short)CameraFlag::USER1);
    startLabel->setCameraMask((unsigned short)CameraFlag::USER1);
